  ...
  <el-container style="background: #ecf5ff;">
    <el-main>
      <el-card class="box-card">
        <div slot="header" style="background-color: #dcdfe6;">
          <el-form ref="timeFilter" :model="timeFilter" :rules="rules" :inline="true" style="width: 60%; margin:auto; padding-top: 22px">
            ...
            <el-form-item>
              <el-button type="primary" @click="filterVisit('timeFilter')">
                Filter
              </el-button>
            </el-form-item>
          </el-form>
...
<script>
import { getTimelineByDay, getTimelineInRange } from '@/api/timeline'
...
export default {
  ...
  methods: {
    filterVisit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.loading = true
          getTimelineInRange(this.timeFilter.startTime, this.timeFilter.endTime)
            .then(resp => {
              if (resp.data != null) {
                this.tableData = resp.data
              } else {
